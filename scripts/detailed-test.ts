...
// Adjust path to point to backend's .env if running from scripts/
...
async function runTests() {
    try {
        ...
        const adminLogin = await fetch(`${API_URL}/auth/login`, {
            ...
            body: JSON.stringify({ email: 'admin@system.com', password: 'password123' }) // Assuming seeded data
        });
        ...
        // If admin doesn't have a branch (Global), lets login as a staff member
        let staffBranchId = null;
        if (!loginBranchId && branches.length > 0) { // Super Admin has null branch
            // Find a staff user
            const staffUser = users.find((u: any) => u.role === 'STAFF' && u.branchDetails);
            if (staffUser) {
                console.log(`   -> Logging in as Staff: ${staffUser.username}...`);
                // Note: we can't easily get the raw password from the DB. 
                // Assuming standard seed password '1234' or 'password123' based on seed file logic?
                // seed-mongo.ts uses `item.pass` for staff.
                // We'll skip actual login and Mock the booking creation using the Branches we found
                // because we can't easily guess the dynamic passwords from `seed-mongo.ts` without reading the file runtime or hardcoding one.
                // Actually, let's just use the Admin context to create a booking? 
                // Booking creation endpoint doesn't strictly check Auth token *middleware* yet (it's open), or checks body.

                // Let's use the first two branches
                ...
                console.log(`✅ Booking Created! LR Number: ${bookingData.booking.lrNumber}`);

                // Update Status
                console.log('   -> Updating Booking Status...');
                const statusRes = await fetch(`${API_URL}/bookings/${bookingData.booking._id}/status`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                ...
                loginBranchId = fromBranch._id; // For Ledger check
            } else {
                console.warn("⚠️ No staff user found.");
            }
        }
