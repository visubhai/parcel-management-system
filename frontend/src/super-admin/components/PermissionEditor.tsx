        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 overflow-y-auto space-y-6">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Primary Branch (Belongs to)</label>
                        <select
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                            {branches.map((b: any) => (
                                ...
                                >
                                    ...
                                    `}>
                                        {formData.allowedBranches.includes(b.name) && <Check className="w-3 h-3" />}
                                    </div>
                                    <input
                                        type="checkbox"
                                        className="hidden"
                                        checked={formData.allowedBranches.includes(b.name)}
                                        onChange={() => toggleBranch(b.name)}
                                    />
                                    <span className={`text-sm font-bold ${formData.allowedBranches.includes(b.name) ? "text-blue-800" : "text-slate-600"}`}>
                                        {b.name}
                                    </span>
                                </label>
                            ))}
